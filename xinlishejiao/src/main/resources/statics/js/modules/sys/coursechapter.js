$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [			
			{ label: 'chapterId', name: 'chapterId', index: 'chapter_id', width: 50, key: true },
			{ label: '', name: 'courseId', index: 'course_id', width: 80 }, 			
			{ label: '', name: 'chapterNumber', index: 'chapter_number', width: 80 }, 			
			{ label: '', name: 'linkUrl', index: 'link_url', width: 80 }, 			
			{ label: '', name: 'courseName', index: 'course_name', width: 80 }			
        ],
		viewrecords: true,
        height: 385,
        rowNum: 10,
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.courseChapter.chapterId == null ? "sys/coursechapter/save" : "sys/coursechapter/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
		...
		getInfo: function(chapterId){
			$.get(baseURL + "sys/coursechapter/info/"+chapterId, function(r){
